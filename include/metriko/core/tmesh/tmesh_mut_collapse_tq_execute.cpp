...
void TmeshMut::collapse_tquad_chain_execute(Tqchain& chain) {
    ...
    for (int i = 0; i < chain.pts.size() - 1; ++i) {
        ...
        std::println("v0: {}, s0: {}, a0: {}", v0, s0, adj0);
        std::println("v1: {}, s1: {}, a1: {}", v1, s1, adj1);
    ...
    std::println("v_bgn: {}, s_bgn: {}, a_bgn: {}", v_bgn, s_bgn, a_bgn);
    std::println("v_end: {}, s_end: {}, a_end: {}", v_end, s_end, a_end);

    vec<int> thids_bgn = consume_pool(l_bgn, s_end, s_bgn == 1);
    vec<int> thids_end = consume_pool(l_end, s_bgn, s_end == 0);
    vec<vec<int>> chains;
    ...
    for (auto& c: chains) {
        std::cout << "chain size: " << c.size() << std::endl;
    }
    std::cout << "thids_end size: " << thids_end.size() << std::endl;

    // todo: need to replace some thdis to other thids (multi to multi)
    ...
    auto extend = [&](const ThalfMut& th, bool ahd, int count_fr, int count_to) -> int {
    ...
    { // leftmost
        ...
        int i = extend(th, ahd, cfr, cto);
        replace(i, thids_bgn);
    }
    { // rightmost
        const auto& th_r = thalfs[chain.thid_r];
        auto cfr = count_adj_tquads(th_r.id);
        auto cto = count_adj_tquads(th_r.twid);
        bool ahd = th_r.loc_fr() == l_end;
        int i = extend(th_r, ahd, cfr, cto);
        replace(i, thids_end);
    }

    for (auto& c: chains) {
        assert(c.size() >= 2);
        auto o = thid_of(thalfs[c.front()].loc_fr(), thalfs[c.back()].loc_to());
