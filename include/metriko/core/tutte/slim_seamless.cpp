...
namespace metriko
{
  ...
  namespace slim
  {
    ...
    IGL_INLINE void buildRhs(metriko::SLIMData& s, const Eigen::SparseMatrix<double> &A);
    IGL_INLINE void add_soft_constraints(metriko::SLIMData& s, Eigen::SparseMatrix<double> &L);
    IGL_INLINE void add_relation_constraints(metriko::SLIMData& s, Eigen::SparseMatrix<double> &L);
    IGL_INLINE double compute_energy(metriko::SLIMData& s, const Eigen::MatrixXd &V_new);
    IGL_INLINE double compute_soft_const_energy(metriko::SLIMData& s,
                                                const Eigen::MatrixXd &V,
                                                const Eigen::MatrixXi &F,
                                                const Eigen::MatrixXd &V_o);

    IGL_INLINE void solve_weighted_arap(metriko::SLIMData& s,
                                        const Eigen::MatrixXd &V,
                                        const Eigen::MatrixXi &F,
                                        Eigen::MatrixXd &uv,
                                        Eigen::VectorXi &soft_b_p,
                                        Eigen::MatrixXd &soft_bc_p);
    IGL_INLINE void update_weights_and_closest_rotations( metriko::SLIMData& s,
                                                          Eigen::MatrixXd &uv);
    IGL_INLINE void compute_jacobians(metriko::SLIMData& s, const Eigen::MatrixXd &uv);
    IGL_INLINE void build_linear_system(metriko::SLIMData& s, Eigen::SparseMatrix<double> &L);
    IGL_INLINE void pre_calc(metriko::SLIMData& s);
    ...
    IGL_INLINE void update_weights_and_closest_rotations(metriko::SLIMData& s, Eigen::MatrixXd &uv)
    {
    ...
    // Seamless relation constraints: E += rel_p * ||x_a - R x_b - t||^2 (2D only).
    // R is a 90-degree rotation, so each row of R has a single +-1 entry and every
    // residual row couples exactly two system variables.
    IGL_INLINE void add_relation_constraints(metriko::SLIMData& s, Eigen::SparseMatrix<double> &L)
    {
      int v_n = s.v_num;
      for (int i = 0; i < s.rel_a.size(); i++)
      {
        int va = s.rel_a(i);
        int vb = s.rel_b(i);
        const Eigen::Matrix2d& R = s.rel_R[i];
        for (int d = 0; d < 2; d++)
        {
          int e      = std::abs(R(d, 0)) > 0.5 ? 0 : 1; // the nonzero column of row d
          double sgn = R(d, e);                         // +-1
          int ia = d * v_n + va;
          int ib = e * v_n + vb;
          L.coeffRef(ia, ia) += s.rel_p;
          L.coeffRef(ib, ib) += s.rel_p;
          L.coeffRef(ia, ib) -= s.rel_p * sgn;
          L.coeffRef(ib, ia) -= s.rel_p * sgn;
          s.rhs(ia) += s.rel_p * s.rel_t(i, d);
          s.rhs(ib) -= s.rel_p * sgn * s.rel_t(i, d);
        }
      }
    }
      ...
      const Eigen::MatrixXd &V_o)
    {
      ...
      for (int i = 0; i < s.rel_a.size(); i++)
      {
        Eigen::Vector2d r = V_o.row(s.rel_a(i)).transpose()
                          - s.rel_R[i] * V_o.row(s.rel_b(i)).transpose()
                          - s.rel_t.row(i).transpose();
        e += s.rel_p * r.squaredNorm();
      }
      return e;
    }
...
IGL_INLINE void metriko::slim_update_weights_and_closest_rotations_with_jacobians(const Eigen::MatrixXd &Ji,
...
IGL_INLINE void metriko::slim_buildA(const Eigen::SparseMatrix<double> &Dx,
...
IGL_INLINE void metriko::slim_precompute(
  const Eigen::MatrixXd &V, 
  const Eigen::MatrixXi &F, 
  const Eigen::MatrixXd &V_init, 
  metriko::SLIMData &data,
  igl::MappingEnergyType slim_energy, 
  ...
  double soft_p)
{
  ...
  metriko::slim::pre_calc(data);
  data.energy = metriko::slim::compute_energy(data,data.V_o) / data.mesh_area;
}

IGL_INLINE Eigen::MatrixXd metriko::slim_solve(metriko::SLIMData &data, int iter_num)
{
  for (int i = 0; i < iter_num; i++)
  {
    ...
    metriko::slim::update_weights_and_closest_rotations(data, dest_res);
    metriko::slim::solve_weighted_arap(data,data.V, data.F, dest_res, data.b, data.bc);

    std::function<double(Eigen::MatrixXd &)> compute_energy = [&](
        Eigen::MatrixXd &aaa) { return metriko::slim::compute_energy(data,aaa); };
