...
#ifndef METRIKO_SLIM_SEAMLESS_H
#define METRIKO_SLIM_SEAMLESS_H
...
struct SLIMData
{
  ...
  // Optional Input: seamless relation constraints (2D only)
  // V_o.row(rel_a(i)) = rel_R[i] * V_o.row(rel_b(i)).transpose() + rel_t.row(i)
  /// Vertex pairs coupled across a seam
  Eigen::VectorXi rel_a, rel_b;
  /// 90-degree rotation per pair
  std::vector<Eigen::Matrix2d> rel_R;
  /// #rel by 2 translation per pair
  Eigen::MatrixXd rel_t;
  /// Weight for enforcing relations as soft constraint
  double rel_p = 0;
