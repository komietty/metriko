...
namespace metriko::visualizer {
    inline void visualize_tedge(
        ...
        for (int i = 0; i < tm.nTE; i++) {
            ...
            for (const Tsgmt &ts: te.sgmts) {
                Row3d p1 = conversion_2d_3d(ts.face, uv, ts.fr);
                Row3d p2 = conversion_2d_3d(ts.face, uv, ts.to);
                ns.emplace_back(p1.x(), p1.y(), p1.z());
                ns.emplace_back(p2.x(), p2.y(), p2.z());
                es.emplace_back(std::array{counter, counter + 1});
                teids.emplace_back(i);
                difx.emplace_back(ts.to.real() - ts.fr.real());
                dify.emplace_back(ts.to.imag() - ts.fr.imag());
